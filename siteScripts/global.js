      ...
      element.addEventListener("click", (e) => {
        ...
        if (appState.siteMapInstance) {
          if (item.route) {
            ...
            } else if (item.route === ROUTES.ARTIST && item.artist) {
              appState.siteMapInstance.navigateTo(ROUTES.ARTIST, { artist: item.artist });
            } else if (item.route === ROUTES.ALL_ARTISTS) {
              appState.siteMapInstance.navigateTo(ROUTES.ALL_ARTISTS);
            }
          } else {
...
renderAlbumSongs: (albumContainer, album, artistName) => {
  ...
  siteMap.bindSongItemEvents(songsContainer);
},
...
bindSongItemEvents: (container) => {
  ...
  container.querySelectorAll(".song-item").forEach((songItem) => {
    ...
    songItem.addEventListener("click", (e) => {
      ...
      if (e.target.closest(".song-actions") || e.target.closest("[data-action]")) {
        return;
      }
      ...
      if (clickCount === 1) {
        clickTimer = setTimeout(() => {
          clickCount = 0;
          // Single click - do nothing or show preview
        }, 300);
    ...
    // Handle single-click play button
    ...
    songItem.querySelectorAll("[data-action]").forEach((actionBtn) => {
      actionBtn.addEventListener("click", (e) => {
        ...
        if (action === 'more') {
          siteMap.showMoreActionsPopover(actionBtn, songData, context);
        } else {
          siteMap.handleSongAction(action, songData, context);
        }
      });
    });
  });
